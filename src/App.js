...
export default function App() {
  ...
  async function handleLikeRepository(id) {
    const repositorie = await api.post(`repositories/${id}/like`);
    const repositorieIndex = repositories.findIndex(p => p.id === id);
    if(repositorieIndex >= 0) {
      repositories[repositorieIndex].likes = repositorie.data.likes;
    }
          ...
          renderItem={({ item: repositorie }) => (
            <View style={styles.repositoryContainer}>
              ...
              <View style={styles.likesContainer}>
                ...
                >
                  {repositorie.likes} curtida{repositorie.likes > 0 ? `s` : null}
                </Text>
              </View>
